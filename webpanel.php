...
// روی موبایل، وقتی صفحه‌کلید باز/بسته می‌شه یا تایپ می‌کنی، پس‌زمینه‌ی
// انیمیشنی‌ی تار (bg-blob) با ادیت/فوکوس روی اینپوت‌ها ترکیب می‌شه و باعث
// هنگ و چشمک‌زدن به رنگ مشکی می‌شه (محدودیت رندر backdrop-filter مرورگرها).
// برای رفع این باگ، تا وقتی کاربر داره تایپ می‌کنه انیمیشن پس‌زمینه رو متوقف می‌کنیم.
document.addEventListener('focusin', e => {
    if (e.target.matches('input, textarea')) document.body.classList.add('input-active');
});
document.addEventListener('focusout', e => {
    if (e.target.matches('input, textarea')) document.body.classList.remove('input-active');
});
